...
int (*jbclient_kernel_steal_ucred)(uint64_t ucredToSteal, uint64_t *orgUcred);
...
#define LIBJAILBREAK_PATH JBROOT_PATH("/basebin/libjailbreak.dylib")

void execute_unsandboxed(void (^block)(void))
{
	// void *libjailbreak = dlopen(LIBJAILBREAK_PATH, RTLD_NOW);
	debug("[dlopen] lj: %s", LIBJAILBREAK_PATH);
	// jbclient_kernel_steal_ucred = dlsym(libjailbreak, "jbclient_root_steal_ucred");
...
int resign(const char* path) {
	void *libjailbreak = dlopen(LIBJAILBREAK_PATH, RTLD_NOW);
   exec_cmd = dlsym(libjailbreak, "exec_cmd");
   debug("[resign] needed %s", path);

	__block int ret = 0;
   execute_unsandboxed(^{

       char* args[] = {"/usr/bin/ldid", "-S","-M", path, NULL};
       ret = exec_cmd(args[0], args);
   });
   debug("[resign] ret %i", ret);
   return ret;
}

// hook dlopen "dirty way"
// void* dlopen_hook(const char* path, int mode) {
	
// 	void* hndl = dlopen(path, mode);
// 	if (hndl) { return hndl; } //hndl is ok, no need to fix

// 	char* err = dlerror();
// 	if (strstr(err, "(code signature invalid ") == NULL) { return hndl; } //error not caused by invalide signature, return


// 	// error caused by invalid signature
// 	// close current handle

// 	dlclose(hndl);


// 	debug("[dlopen_hook] resigning: %s", path);
// 	resign(path);
...
// {
// 	void* hndl = dlopen_from(path, mode, addressInCaller);
// 	if (hndl) { return hndl; } //hndl is ok, no need to fix
...
// 	debug("[dlopen_from_hook] resigning: %s", path);
...
int csops_hook(pid_t pid, unsigned int ops, void *useraddr, size_t usersize)
{
	...
	if (ops == CS_OPS_STATUS) {
		if (useraddr && usersize == sizeof(uint32_t)) {
			...
			*csflag |= CS_DEBUGGED | CS_INVALID_ALLOWED | CS_GET_TASK_ALLOW | CS_VALID | CS_SIGNED | CS_ADHOC | CS_PLATFORM_BINARY | CS_NVRAM_UNRESTRICTED | CS_ENTITLEMENTS_VALIDATED;
			// }
		}
	}
	return rv;
