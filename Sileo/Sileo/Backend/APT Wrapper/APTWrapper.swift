...
class APTWrapper {
    ...
    public class func performOperations(installs: [DownloadPackage],
        ...
        DispatchQueue.global(qos: .default).async {
            ...
            // Legacy rootful tweaks install fine but carry SHA-1-only signatures
            // that iOS 15 refuses to load. Fix them up before we report success,
            // streaming into the same log the user is already watching.
            if status == 0 {
                LegacyTweakPatcher.patch(installed: installs) { message in
                    outputCallback(message, debugFD)
                }
            }
