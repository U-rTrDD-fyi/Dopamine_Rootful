...
struct APTOperation: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // APT qualifies foreign-architecture packages in its output as
        // "name:arch" - a legacy rootful tweak comes back as
        // "com.example.tweak:iphoneos-arm". Sileo's package database is keyed by
        // the bare identifier, so without stripping the qualifier every lookup
        // misses and the operation list fails to reconcile ("Install Identifier
        // Mismatch"). Debian package names can never contain ':', so taking
        // everything before the first one is unambiguous.
        let qualifiedPackageID = try values.decode(String.self, forKey: .packageID)
        self.packageID = String(qualifiedPackageID.prefix(while: { $0 != ":" }))

        self.version = try values.decode(String.self, forKey: .version)
        self.type = try values.decode(OperationType.self, forKey: .type)
