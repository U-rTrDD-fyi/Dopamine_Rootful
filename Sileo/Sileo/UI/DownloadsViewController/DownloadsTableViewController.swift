...
class DownloadsTableViewController: SileoViewController {
    ...
    public func transferToInstall() {
        ...
        if !earlyBreak {
            // Open the log automatically instead of hiding it behind "Show
            // Details". Legacy tweaks report here what had to be re-signed, and
            // an install that succeeds silently while the tweak never loads is
    ...
    private static let detailsHeaderHeight: CGFloat = 44

    /// Build the "Install Log" bar that sits above the log text, with a Copy
    /// action. Added programmatically so the storyboard's own constraints on the
    /// text view are left untouched.
    private func installDetailsHeaderIfNeeded(in detailsView: UIView) {
        let headerTag = 0x510E0
        if detailsView.viewWithTag(headerTag) != nil { return }
        ...
        title.textColor = .sileoLabel
        header.addSubview(title)

        let copyButton = UIButton(type: .system)
        copyButton.frame = CGRect(x: detailsView.bounds.width - 74, y: 0, width: 58, height: Self.detailsHeaderHeight)
        copyButton.autoresizingMask = [.flexibleLeftMargin]
        copyButton.setTitle(String(localizationKey: "Copy"), for: .normal)
        copyButton.titleLabel?.font = .preferredFont(forTextStyle: .body)
        copyButton.addTarget(self, action: #selector(copyDetailsLog(_:)), for: .touchUpInside)
        header.addSubview(copyButton)

        detailsView.addSubview(header)
        detailsView.bringSubviewToFront(header)

        // Push the log text below the header rather than resizing it, so the
        // storyboard layout keeps working.
        detailsTextView?.contentInset = UIEdgeInsets(top: Self.detailsHeaderHeight, left: 0, bottom: 0, right: 0)
    }

    @objc func copyDetailsLog(_ sender: Any?) {
        UIPasteboard.general.string = detailsAttributedString?.string ?? detailsTextView?.text ?? ""
    }
    ...
    @IBAction func showDetails(_ sender: Any?) {
        ...
        // Occupy only the lower part of the sheet so the package row and its
        // progress bar stay visible above the log, instead of the log taking
        // over the whole screen.
        let logTopOffset = (self.view.bounds.height * 0.42).rounded()
        detailsView.alpha = 0
        detailsView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        detailsView.frame = CGRect(x: 0,
                                   y: logTopOffset,
                                   width: self.view.bounds.width,
                                   height: self.view.bounds.height - logTopOffset)
        detailsView.layer.cornerRadius = 16
        detailsView.layer.maskedCorners = [.layerMinXMinYCorner, .layerMaxXMinYCorner]
        detailsView.clipsToBounds = true

        self.view.addSubview(detailsView)
        self.installDetailsHeaderIfNeeded(in: detailsView)
